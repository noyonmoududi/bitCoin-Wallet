...
function getInputs(utxos) {
    return new Promise(async (resolve,reject) => {
        ...
        for(let k=0;k<utxos.length;k++){
            let utxo = utxos[k];
            if (utxo.status.confirmed >= 1) {
                try{
                    let urlTxes = await axios.get("https://blockstream.info/testnet/api/tx/"+utxo.txid+"/hex");
...
exports.send = (fromPublicKey,privateKey, outputs,fee=0) => {
    return new Promise((resolve, reject) => {
        ...
        if(!validator.validate(fromPublicKey,'BTC','testnet')){
            return reject({
                'error' : 1,
                'error_code' : 550,
        ...
        for(let i=0;i<outputs.length;i++){
            ...
            if(!validator.validate(output.address,'BTC','test')){
                return reject({
                    'error' : 1,
                    'error_code' : 550,
        ...
        axios.get("https://blockstream.info/testnet/api/address/"+fromPublicKey+"/utxo").then(async resp => {
            try {
                const utxos = resp.data
                let key;
                try{
                    var wallet = await Wallet.getWallet(privateKey);
                    console.log(wallet,fromPublicKey)
                ...
                let tx = new bitcoin.Psbt({ network: bitcoinNetwork });
                let availableSat = 0;
                utxos.forEach(input => {
                ...
                axios.post('https://api.blockcypher.com/v1/btc/test3/txs/push', {tx: tx.extractTransaction().toHex() }).then(resp => {
                    return resolve({
                        'success':true,
                        'error': 0,
